...
parser.add_argument('--num-workers', type=int, default=9, help='number of workers')
...
parser.add_argument('--train-path', type=str, default='./train', help='path of training data')
parser.add_argument('--val-path', type=str, default='./val1', help='path of validation data')
parser.add_argument('--test-path', type=str, default='./test', help='path of test data')
parser.add_argument('--u-mask-path', type=str, default='./mask/undersampling_mask/mask_4.00x_acs24.mat', help='undersampling mask')
...
# save path
parser.add_argument('--model_save_path', type=str, default='./checkpoints/', help='save path of trained model')
parser.add_argument('--loss_curve_path', type=str, default='./runs/loss_curve/', help='save path of loss curve in tensorboard')
# others
...
def solvers(rank, ngpus_per_node, args):
    ...
    for epoch in range(start_epoch + 1, args.num_epochs + 1):
        ...
        if rank == 0:
            ...
            train_loss_diff.append(train_loss)
            val_loss_diff.append(val_loss)
            train_loss_diff[epoch] = train_loss - train_loss_diff[epoch - 1]
            val_loss_diff[epoch] = val_loss - val_loss_diff[epoch - 1]

            writer.add_scalars('loss', {'train_loss': train_loss, 'val_loss': val_loss}, epoch)
            writer.add_scalars('loss_diff', {'train_loss_diff': train_loss_diff[epoch], 'val_loss_diff': val_loss_diff[epoch]}, epoch)
            # save checkpoint
            checkpoint = {
                'epoch': epoch,
                'lr': lr,
